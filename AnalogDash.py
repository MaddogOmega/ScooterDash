...
import pygame 
import pickle
import RPi.GPIO as GPIO #rpi-lgpio delete RPi-GPIO first
import spidev
import serial
import time

#variables lots of them
angle = 54  # 54 is zero for speed 1 = 15mph  -90 = 40mph, -181 = 65mph x-54  (54-3.6 x speed mph)
anglef = 3 # 33 is empty -27 full 60 span fuel 
anglev = 122 #92 is 8 volts, 122 is 12 Volts, 150 = 16volts
anglet = 225 # 212.5 is C, 241 is 210, 272 is H not good
current_odom = 0
disttrvled = float(0)
GPGGA_buffer = 0
gpgga_info = "$GPGGA,"
...
fuel = 0
temp = 0
battery = 7 #prevent shutdown on first scan... hopefully
...
text_font_medium = pygame.font.SysFont("URWGothic-Book", 54)
...
while running:
    ...
    background = pygame.image.load("green5.png").convert_alpha()
    spdneedle = pygame.image.load("greeneedle.png").convert_alpha()
    smallneedle = pygame.image.load("smallneedle.png").convert_alpha()
    rect2 = background.get_rect()
    rect2.topleft = (0, 0)
    
    # Clear the screen
    screen.fill((255, 255, 255))  # White background
    screen.blit(background, rect2)
    ...
    if hb_state == 1:
        hb = pygame.image.load("greenhb.png").convert_alpha()
        screen.blit(hb, (354, 925)) #location
    if turn_state == 1:
        tn = pygame.image.load("greenturn.png").convert_alpha()
        screen.blit(tn, (618, 925)) #location
        
    # Select trip, oil or odometer or display
    if current_odom == 0:
        draw_text(f"Odom {odom: .1f}", text_font_medium, (250, 250, 250), 418, 630)
    elif current_odom == 1:
        draw_text(f"Trip A {tripA: .1f}", text_font_medium, (250, 250, 250), 418, 630)
    elif current_odom == 2:
        draw_text(f"Trip B {tripB: .1f}", text_font_medium, (250, 250, 250), 418, 630)
    elif current_odom == 3:
        draw_text(f"Oil {oil: .1f}", text_font_medium, (250, 250, 250), 418, 630)
    
    # Set angles
    angle = 54 - (speed * 3.6) # 54 is zero for speed 1 = 15mph  -90 = 40mph, -181 = 65mph x-54 (54-3.6 x speed mph)
    anglef = 33 - (fuel * .6) # 33 is empty -27 full 60 span fuel 
    anglev = 7.17 * (battery + 5) # 92 is 8 volts, 122 is 12 Volts, 150 = 16volts
    anglet = 1.15 * temp # 212.5 is C, 241 is 210, 272 is H not good
    if anglet < 212: # prevents needle from being off gauge
        anglet = 212
    
    # Images for all below
    blitRotate(screen, spdneedle, (540, 530), (418, 72), angle) # Speed needle
    blitRotate(screen, smallneedle, (540, 540), (3, 337), anglef) # Fuel needle
    blitRotate(screen, smallneedle, (540, 540), (3, 337), anglev) # Volts needle
    blitRotate(screen, smallneedle, (540, 540), (3, 335), anglet) # Temp needle
